...
namespace Game.Monsters.SpellDemon
{
    public class AttackState : AttackStateBase<SpellDemonController>
    {
        ...
        public override void OnEnter()
        {
            ...
            ,originalMaterial = controller.MySkinnedMeshes[0].material};
            originalScale = controller.transform.localScale;
            base.OnEnter();

            //This paremetars are examples,so please change it to your preference!!
            if(attackEndNomTime == 0f) StateFieldSetter.AttackStateFieldSet<SpellDemonController>(controller, this, clipLength, 25, 0.5f);      
        }
        public override void OnUpdate()
        {
            if (!isTheBiggest)
            {
               if(target.statusCondition.DemonCurse.isActive)ChangeStatus();
            }
                if (controller.isDead) return;
            //Debug.Log($"[ƒAƒjƒó‘Ô] name: {state.fullPathHash}, time: {state.normalizedTime}, looping: {state.loop}");
            if (!isAttacking)
            {
                isAttacking = true;
                Attack();
        ...
        void SetMaterial(Material newMaterial)
        {
            ...
            for (int i = 0; i < rendererInfo.renderes.Count; i++)
            {
                ...
                foreach (var materials in rendererInfo.meshMaterials)
                {
